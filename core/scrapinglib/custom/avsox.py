...
class Avsox(Parser):
    ...
    def __init__(self, _session = None):
        super(Avsox, self).__init__()
        qurySiteTree = self.getHtmlTree('https://tellme.pw/avsox')
        self.site = self.getTreeElement(qurySiteTree, '//div[@class="container"]/div/a/@href')
        if self.site is None:
            self.site = 'https://avsox.com/'
    ...
    def queryNumberUrl(self, number: str):
        ...
        self.searchtree = self.getHtmlTree(self.site + '/cn/search/' + number)
        ...
        if result1 == '' or result1 == 'null' or result1 == 'None' or result1.find('movie') == -1:
            self.searchtree = self.getHtmlTree(self.site + '/cn/search/' + number.replace('-', '_'))
            result1 = self.getTreeElement(self.searchtree, '//*[@id="waterfall"]/div/a/@href')
            if result1 == '' or result1 == 'null' or result1 == 'None' or result1.find('movie') == -1:
                self.searchtree = self.getHtmlTree(self.site + '/cn/search/' + number.replace('_', ''))
                result1 = self.getTreeElement(self.searchtree, '//*[@id="waterfall"]/div/a/@href')
